...
export function activate(context: vscode.ExtensionContext) {
  ...
  const openDashboard = vscode.commands.registerCommand(cmdId, () => {
    const panel = vscode.window.createWebviewPanel('changesDashboard', 'GitLineDiffs', vscode.ViewColumn.One, {
      enableScripts: true,
    });
  ...
  let authorChanges = new Map<string, AuthorChanges>();
  ...
  const updateChangedFilesCount = async () => {
    ...
    for (let repo of api.repositories) {
      ...
      logs.forEach(log => commitSummaries.push({
        message: log.message,
  ...
  function getWebviewContent(fileChanges: FileChange[], themeKind: vscode.ColorThemeKind) {
    ...
    const sortedFileChanges = [...fileChanges].sort((a, b) => (b.added + b.removed) - (a.added + a.removed));
    const rows = sortedFileChanges.map(change => `
